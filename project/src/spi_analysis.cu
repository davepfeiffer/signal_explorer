...
// AI slop for cuda error reporting
...
__device__ void derrivative(float *signal, int samples, float *output //
) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < samples - 1) {
...
void process_signal(const std::vector<float> &data, float noise_scale,
